...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  const post = await getPost('multiple-line-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/multiple-line-graph-maker`,
      languages: {
        'en': `${url}/en/multiple-line-graph-maker`,
        'zh': `${url}/zh/multiple-line-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
