...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  const post = await getPost('multi-bar-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/multi-bar-graph-maker`,
      languages: {
        'en': `${url}/en/multi-bar-graph-maker`,
        'zh': `${url}/zh/multi-bar-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/multi-bar-graph-maker`,
    }
  }
}
