...
import {SmoothLineGraph} from "@/components/SmoothLineGraph"
...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  const post = await getPost('smooth-line-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/smooth-line-graph-maker`,
      languages: {
        'en': `${url}/en/smooth-line-graph-maker`,
        'zh': `${url}/zh/smooth-line-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/smooth-line-graph-maker`,
    }
  }
}
