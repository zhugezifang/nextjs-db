...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  const post = await getPost('triple-bar-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/triple-bar-graph-maker`,
      languages: {
        'en': `${url}/en/triple-bar-graph-maker`,
        'zh': `${url}/zh/triple-bar-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/triple-bar-graph-maker`,
    }
  }
}
