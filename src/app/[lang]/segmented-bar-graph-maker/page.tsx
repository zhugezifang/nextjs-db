...
export default async function HandPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  const post = await getPost('segmented-bar-graph-maker', lang) as unknown as { 
    title: string; 
    desc: string;
    contentHtml: string; 
  }
...
export async function generateMetadata({ 
  ...
  return {
    ...
    alternates: {
      canonical: `${url}/${lang}/segmented-bar-graph-maker`,
      languages: {
        'en': `${url}/en/segmented-bar-graph-maker`,
        'zh': `${url}/zh/segmented-bar-graph-maker`,
      },
    },
    openGraph: {
      title: post.title,
      description: post.desc,
      url: `${url}/${lang}/segmented-bar-graph-maker`,
    }
  }
}
